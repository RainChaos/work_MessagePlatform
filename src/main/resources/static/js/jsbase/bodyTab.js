...
//后退键
	$("#goback").on("click",function(){
		...
		//删除tab后重置session中的menu和curmenu
		liIndex = $("#top_tabs li").length;

if(liIndex<=1||$("#top_tabs li.layui-this cite").text()=="首页" ){
	layer.msg("这是首页，不能再后退了哦");
	return false;
	}
		var menu = JSON.parse(window.sessionStorage.getItem("menu"));
		//获取被删除元素
		...
		menu.splice((liIndex-1), 1);
		window.sessionStorage.setItem("menu",JSON.stringify(menu));
		//$(this).hasClass("layui-this")
		//element.tabDelete("bodyTab",$(this).parent("li").attr("lay-id")).init();
		// alert($(this).parent("li").attr("lay-id"));
			element.tabDelete("bodyTab",$("#top_tabs li.layui-this").attr("lay-id")).init();
		new Tab().tabMove();
